...
from .AllException import AccessDenied,AdminKeyIsNotProvided,JWTTokenCheckException, LoginDoesNotExist


def add_owner_exception_handler(api: FastAPI) -> None:
    ...
    @api.exception_handler(LoginDoesNotExist)
    async def unicorn_exception_handler(request: Request, exc: LoginDoesNotExist):
        return JSONResponse(
            status_code=400,
            content={"message": "This login doesn't exist"},
        )